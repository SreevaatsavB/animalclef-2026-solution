...
The motivation for local feature matching comes from HotSpotter~\cite{crall2013hotspotter},
which identifies patterned animals by matching distinctive spot and scale markings
using local descriptors. Texas Horned Lizards in particular have highly distinctive
scale arrangements and spot patterns that are stable across re-sightings --- exactly
the kind of signal that patch-level matching is designed to capture. We extend this
idea to all four species with three different extractors.
...
LNBNN (Local Naive Bayes Nearest Neighbor) is HotSpotter's matching score, so
replacing BFMatcher with it seemed reasonable. Score collapsed to 0.107. The issue
is structural: LNBNN measures how much closer a descriptor is to its nearest match
than to the background distribution. In our pipeline the background pool is the
top-$K=100$ candidate images, which are all similar-looking animals from the same
species, so background distance ends up nearly identical to match distance for most
descriptors. Almost all LNBNN deltas are zero or negative, local scores collapse to
zero, and the ensemble falls back to global cosine similarity with badly miscalibrated
thresholds. LNBNN needs a large, diverse background database; it breaks when the
candidate set is a curated shortlist of near-duplicates from the same species.
